...
void usage() {
	printf("syntax : 1m_block <web list>.txt\n");
...
unordered_map<string, bool> intxt;
int jump[300];
...
static u_int32_t blockchk (struct nfq_data *tb)
{
	...
	{		
		...
		unsigned char* datahost = (unsigned char*) malloc (sizeof(unsigned char) * datahostlen);
		memcpy(datahost, tcpdata + pos + 8, datahostlen);
		string tem(reinterpret_cast<char*>(datahost));
		free(datahost);	
		if(intxt[tem])
			return -1;	
	}
